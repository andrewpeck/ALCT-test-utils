################################################################################
# TesterBoard.py -- Tools for Verifying ASIC delays using the large external
#                   test board
################################################################################
...
def SetFIFOChannel(ch,startdly):
    ...
    WriteDR( ch | startdly << 6, WrAddF.length)

def SetFIFOMode(mode):
    WriteIR(WrParamF.val, V_IR)
    WriteDR((0x8 | mode) & 0xF, WrParamF.length)

def SetFIFOReset():
    WriteIR(WrParamF.val, V_IR)
    WriteDR(8,WrParamF.length)
    WriteIR(WrParamF.val, V_IR)
    WriteDR(FIFO_RESET,WrParamF.length)
    WriteIR(WrParamF.val, V_IR)
    WriteDR(8,WrParamF.length)

def SetFIFOWrite():
    WriteIR(WrParamF.val, V_IR)
    WriteDR(FIFO_WRITE,WrParamF.length)

def SetFIFORead():
    WriteIR(WrParamF.val, V_IR)
    WriteDR(FIFO_READ,WrParamF.length)

def SetFIFOReadWrite():
    WriteIR(WrParamF.val, V_IR)
    WriteDR(FIFO_WRITE | FIFO_READ,WrParamF.length)

def FIFOClock():
    WriteIR(WrFIFO[val], V_IR)
    WriteDR(0x1,WrFIFO[length])

def ReadFIFOCounters():
    WriteIR(RdCntReg[val], V_IR)
    return(ReadDR(0x0,RdCntReg[length]))

def SetFIFOValue(val):
    WriteIR(WrDatF.val, V_IR)
    WriteDR(val,WrDatF.length)
...
def SetTestBoardDelay(delay):
    ...
    WriteDR(0xFF & FlipByte((255-delay) & 0xFF) ,WrDlyF[length])

    WriteIR(WrParamF.val, V_IR)
    WriteDR(0x8,WrParamF.length)
...
def ReadFIFOValue():
    ...
    result = ReadDR(0,RdDataReg.length)
    return(result)
...
def ReadFIFOfast(numwords, cntrs, alcttype):
    ...
    for i in range(numwords):
        WriteDR(0x1,WrFIFO[length])     # FIFOClock
